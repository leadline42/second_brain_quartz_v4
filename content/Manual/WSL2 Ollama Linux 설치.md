...
외부 접근을 위해서는 
Environment="PATH=$PATH" 에 "OLLAMA_HOST=0.0.0.0:11434"를 추가해준다.

# WSL2에 CUDA설치 

## 1. 아래 다운로드 링크의 안내 사항에 따라 설치
[https://developer.nvidia.com/cuda-12-6-3-download-archive?target_os=Linux&target_arch=x86_64&Distribution=WSL-Ubuntu&target_version=2.0&target_type=deb_network](https://developer.nvidia.com/cuda-12-6-3-download-archive?target_os=Linux&target_arch=x86_64&Distribution=WSL-Ubuntu&target_version=2.0&target_type=deb_network)
...
상세내용은 [Link_Ubuntu_Q&A](https://askubuntu.com/a/1210812)를 참고한다.
...
## 1. 아래 다운로드 링크의 안내 사항에 따라 설치
[https://developer.nvidia.com/cudnn-downloads?target_os=Linux&target_arch=x86_64&Distribution=Ubuntu&target_version=24.04&target_type=deb_network&Configuration=Full](https://developer.nvidia.com/cudnn-downloads?target_os=Linux&target_arch=x86_64&Distribution=Ubuntu&target_version=24.04&target_type=deb_network&Configuration=Full)
...
포트 포워딩 스크립트는 윈도우에서 실행하므로 ports_wsl.ps1 확장자로 작성
내용은 아래 참고
...
그리고 Powershell에서 작성된 스크립트 실행 .\ports_wsl.ps1
...
마지막으로 wsl 포트 포워딩 설정 (윈도우에서 실행, WSL2_IP는 위에서 ifocnfig를 통해서 확인 가능)
```powershell
